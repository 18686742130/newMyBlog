...
<div class="MainForm">
<form id="BlogForm" action="user/createBlog" method="POST">
<input id="hdn_blog_id" name="draft" value="0" type="hidden">
	...
	<select name="typeID">
		<?php
		 for($i=0;$i<count($type);$i++){
			 ?>
		<option selected="selected" value=<?php echo $type[$i]->type_id ?>><?php echo $type[$i]->type_name ?></option>
		<?php
		 }
		  ?>
		</select>
	<a href="blogType.ejs" onclick="return confirm('是否放弃当前编辑进入分类管理？');">分类管理»</a>
	</td>
  </tr>
  <tr><td class='t'>内容（必填） 
  ...
  <tr>
    <td>
		<textarea name="content" id="ta_blog_content" style="width:750px;height:300px;"></textarea></td>
  </tr>
  <tr class="option">
